...
pub fn result() {
  ...
  parsing_the_big_grid();
}

fn parsing_the_big_grid() {
  ...
  let diagonal_quads = diagonal_quad_finder(grid);
  let mut diagonal_products = vec_of_vecs_productizer(diagonal_quads);
  diagonal_products.sort();
  println!("{:?}", diagonal_products);

  let highest_diagonal_sum = diagonal_products.last().unwrap();
  println!("Highest Diagonal Product: {}", highest_diagonal_sum);
  // 70600674
}

fn vec_of_vecs_productizer(quad_collection: Vec<Vec<i32>>) -> Vec<i32> {
  let mut quad_products: Vec<i32> = vec![];
    for quad in quad_collection {
      let product = quad.iter().fold(1, |acc, x| acc * x);
      quad_products.push(product);
    }
    quad_products
}
